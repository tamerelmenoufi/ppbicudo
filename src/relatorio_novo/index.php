    ...
    if($_POST['deletar']){
      ...
      if($_SESSION['modelo_relatorio']){
        $q = "UPDATE relatorio_modelos set 
                                          {$modelo_campo} = JSON_REMOVE({$modelo_campo}, JSON_UNQUOTE(JSON_SEARCH({$modelo_campo}, 'one', '{$_POST['deletar']}')))              where codigo = '{$_SESSION['modelo_relatorio']}'";
        mysqli_query($con, $q);
      }
      // echo $query;
...
<?php
    ...
    if($_POST['acao'] == 'relatorio'){
      ...
      if($_POST['codigo_relatorio']){
        $query = "UPDATE relatorio_modelos set nome = '{$_POST['nome_relatorio']}', data = '{$_POST['data_relatorio']}', {$modelo_campo} = '{$registros}' where codigo = '{$_POST['codigo_relatorio']}'";
        mysqli_query($con, $query);
        $_SESSION['modelo_relatorio'] = $_POST['codigo_relatorio'];
      }else{
        $query = "INSERT INTO relatorio_modelos set nome = '{$_POST['nome_relatorio']}', data = '{$_POST['data_relatorio']}', {$modelo_campo} = '{$registros}', origem = '{$_POST['origem']}'";
        mysqli_query($con, $query);
                        ...
                        <div class="input-group">
                          ...
                          <select class="form-select" id="relatorio_anexar">
                            ...
                            <?php
                                ...
                                while($s = mysqli_fetch_object($r)){
                            ?>
                            <option value="<?=$s->codigo?>-registros"><?="{$s->nome} - {$s->mes}/{$s->ano}"?></option>
                            <option value="<?=$s->codigo?>-devolucoes" style="color:red"><?="{$s->nome} - {$s->mes}/{$s->ano} Devoluções"?></option>
                            <?php
                                }
                            ?>
                          </select>
...
<?php
  if($where){
?>
<div style="display: flex; justify-content: flex-end; position:fixed; bottom:0; left:0; right:0; height:/*65*/90px; border:solid 0px red; padding-top:10px; padding-bottom:10px; padding-left:30px; padding-right:100px; background-color:#fff; z-index:100;">
...
<tbody>
  ...
  <tr class="calculaTitulos">
    ...
    if($modelo_campo == 'devolucoes'){
    ?>
    <td class="col-1 text-nowrap"><span class="vermelho">R$ <?=$t->totalValorPedidoXquantidade?></span></td>
    <td class="col-1 text-nowrap"><span class="vermelho">R$ <?=$t->totalCustoEnvio?></span></td>
    <td class="col-1 text-nowrap"><span class="vermelho">R$ <?=$t->totalPrecoCusto?></span></td>
    ...
    <td class="col-1 text-nowrap"><span class="rodapeComissao"></span></td> 
    <td class="col-1 text-nowrap"><span class="rodapeLucro"></span></td>    
    <?php
    }
    ?>
