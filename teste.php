...
$arq = 'arquivo_existente2.xlsx';
// Utilizacao:
$xlsx = new SimpleXLSX($arq);
$dados = $xlsx->rows();
$n = count($dados);

 echo "<div class='continer' style='margin-top:20px;'>";
 echo "<h1>Importa̤̣o de Registros (excel)</h1>";
 echo "<div class='table-responsive' style='height:600px;'>";
...
for($i=0;$i<10/*$n*/;$i++){
     ...
     if($i==0){
         ...
         for ($j = 0; $j < count($dados[$i]); $j++) {
             ...
             echo "<th CamposInsert>".$LigarCampos."</th>";
             
         }
             echo "<th><button SalvarImportacao type='button' class='btn btn-success'><i class='fa fa-check' aria-hidden='true'></i></button></th>";
         echo "</tr>";
     }
     
     echo "<tr linhas linha".$i.">";

        for($j = 0; $j < count($dados[$i]); $j++){
             echo "<td dados='1'>".($dados[$i][$j])."</td>";
        }
